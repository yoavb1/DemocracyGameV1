...
const firebaseConfig = {
  apiKey: "AIzaSyCJi5etsFY_x7N3pBv8BmKlcF9jPFzYqls",
  authDomain: "democracygamev1-b087a.firebaseapp.com",
  projectId: "democracygamev1-b087a",
  storageBucket: "democracygamev1-b087a.firebasestorage.app",
  messagingSenderId: "392652481738",
  appId: "1:392652481738:web:b588fd6edde0f35718c033"
};
...
class PromiseThrottle {
  ...
  Add(a) {
    return new Promise((e, t) => {
      this._queue.push({ func: a, resolve: e, reject: t });
...
function PostBroadcastMessage(e) {
  broadcastChannel && setTimeout(() => broadcastChannel.postMessage(e), 3000);
}
...
function IsUrlInLazyLoadList(e, t) {
  if (t)
    try {
      for (const a of t) if (new RegExp(a).test(e)) return true;
    } catch (e) {
...
function WriteLazyLoadListToStorage(e) {
  return "undefined" == typeof localforage
    ? Promise.resolve()
    : localforage.setItem(LAZYLOAD_KEYNAME, e);
}
...
async function GetMainPageUrl() {
  ...
  for (const t of e) {
    let e = t.url;
    if (
      (e = e.startsWith(self.registration.scope)
        ? e.substring(self.registration.scope.length)
        : e) &&
      "/" !== e
    )
...
async function CreateCacheFromFileList(t, a, o) {
  const e = await Promise.all(
    a.map((e) => networkThrottle.Add(() => fetchWithBypass(e, o)))
  );
  let s = !0;
  for (const r of e)
    r.ok ||
      ((s = !1),
      console.error(CONSOLE_PREFIX + "Error fetching '" + r.url + "' (" + r.status + " " + r.statusText + ")"));
  ...
  try {
    ...
    throw (
      (console.error(CONSOLE_PREFIX + "Error writing cache entries: ", e),
      caches.delete(t),
      e)
    );
  }
...
async function UpdateCheck(e) {
  try {
    ...
    if (i) {
      ...
      d
        ? (console.log(CONSOLE_PREFIX + "All resources saved, update ready"), BroadcastUpdateReady(o))
...
async function HandleFetch(t, e) {
  ...
  const r = await Promise.all([fetch(t.request), ReadLazyLoadListFromStorage()]),
    i = r[0],
    c = r[1];
  if (IsUrlInLazyLoadList(t.request.url, c))
    try {
